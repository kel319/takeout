...
public class RedisUtil {
    ...
    private static final long NULL_TIME = 30;
    ...
    public <T> T get(String key, JavaType javaType) {
        try {
            ...
            log.warn("类型转换异常,error: {}",e.getMessage());
            return null;
        } catch (Exception e) {
    ...
    public void set(String key) {
        if (key != null) {
            try {
                redisTemplate.opsForValue().set(key,NULL_VALUE,NULL_TIME,TimeUnit.SECONDS);
            } catch (Exception e) {
    ...
    public boolean setDistributedLock(String key,String value,Integer timeSeconds) {
        try {
            ...
            if (Boolean.TRUE.equals(lock)) {
                ...
                }, 0, timeSeconds / 2, TimeUnit.SECONDS);
                future.thenRun(() -> scheduled.cancel(false));
                future.thenRun(() -> log.info("自动续期取消"));
            }
